...
{
    public class PlotInfoParser
    {
        public PlotInfoParser()
        {
        ...
        private PhaseInfo _currentPhase = null;

        public void ParseLine(string line)
        {
            var plotInfo = PlotInfo;

            if (line.StartsWith(LogParser.StartingPlotToken))
            {
                var startLine = line.Substring(LogParser.StartingPlotToken.Length + 1).Trim();
            ...
            {
                if (_currentPhase != null) _currentPhase.LogLineCount++;
            }
        }
